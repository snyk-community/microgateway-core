...
function _sendTargetRequest(sourceRequest, sourceResponse, plugins, startTime, correlation_id, cb) {
  ...
  function setTargetHeader(name, comma, fn) {
    if (_configured(config, name)) {
      if (comma && target_headers[name]) {
        target_headers[name] += ', ' + fn();
        return;
      }
      target_headers[name] = fn();
    }
  ...
  var target_path;
  {
    const retarget_header = config.edgemicro && config.edgemicro.retarget_header;
    const forwarded_url = retarget_header && target_headers[retarget_header];
    if (forwarded_url) {
      const parsedHost = proxy.parsedUrl.hostname
      target_path = forwarded_url.slice(forwarded_url.indexOf(parsedHost) + parsedHost.length)
    } else {
      target_path = proxy.parsedUrl.pathname + reqUrl.pathname.slice(proxy.basePathLength); // strip leading base_path
