...
function _sendTargetRequest(sourceRequest, sourceResponse, plugins, startTime, correlation_id, cb) {
  ...
  var target_path
  // TODO-CF target path override
  if (target_headers['x-cf-forwarded-url']) {
    const cf_url = target_headers['x-cf-forwarded-url']
    const parsedHost = proxy.parsedUrl.hostname
    target_path = cf_url.slice(cf_url.indexOf(parsedHost) + parsedHost.length)
  } else {
