...
// TODO-CF detect whether running in container that is capturing stdout/stderr
const logToConsole = process.env.VCAP_APPLICATION
...
module.exports.init = function init(stubConfig) {
  ...
  var rotation = 0;
  const baseFileName = util.format('edgemicro-%s-%s', os.hostname(), uid);
  ...
  const writeLogRecordToFile = function (record) {
    ...
    if (Date.now() > nextRotation) {
      rotation++;
      const baseFileName = util.format('edgemicro-%s-%s-%d', os.hostname(), uid, rotation);
  ...
  const writeLog = function (level, obj, msg) {
    ...
    return writeLogRecord(serializeLogRecord(level, logConfig.level, obj, msg));
  }
  logger = {
  ...
  const writeLogRecord = logToConsole ? (record) => {
      if (record) {
